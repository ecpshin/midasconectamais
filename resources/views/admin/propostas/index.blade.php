                ...
                <div class="row flex flex-row justify-between">
                    <div class="col-lg-3 mb-3">
                        <a href="{{ route('admin.propostas.create') }}"
                            class="flex h-[35px] w-[118px] flex-row items-center justify-center gap-2 rounded-full bg-emerald-900 px-8 py-2 text-white shadow-md shadow-emerald-950 hover:bg-emerald-600">
                            <i class="bi bi-plus-circle"></i>
                            Adicionar
                        </a>
                    </div>
                    <div class="col-lg-9 mb-3 flex flex-col items-end">
                        <form action="{{ route('admin.propostas.filtrar') }}" method="post" class="flex flex-row items-center justify-between gap-3">
                            @csrf
                            <input type="date" name="inicio" value="{{ old('inicio', date('Y-m-d')) }}" class="rounded-full border-slate-300 text-xs">
                            <input type="date" name="fim" value="{{ old('fim', date('Y-m-d')) }}" class="rounded-full border-slate-300 text-xs text-slate-700">
                            <button type="submit"
                                class="rounded-lg bg-gradient-to-br from-blue-800 to-slate-600 px-8 py-1 text-stone-50 shadow-sm hover:bg-gradient-to-b hover:from-blue-700 hover:to-sky-600 hover:text-slate-100">Filtrar</button>
                        </form>
                    </div>
                </div>
