<section>
    <header>
        ...
        <p class="mt-1 text-sm text-gray-600">
            Assegure-se de que está utilizando uma senha longa para se manter seguro.
        </p>
    </header>
    ...
    <form method="post" action="{{ route('password.update') }}" class="mt-6 space-y-6">
        ...
        <div class="flex items-center gap-4">
            ...
            @if (session('status') === 'password-updated')
                <p x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 2000)" class="text-sm text-gray-600">{{ __('Salva.') }}</p>
            @endif
        </div>
    </form>
</section>
